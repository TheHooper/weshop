 ...
 */
(function (f) {
    var d = f.ajax, e = [], l = [], u = [], n = /=\?(&|$)/, p = (new Date()).getTime();
    ...
    function m(w) {
        ...
        try {
            ...
            f(document).trigger("xmlParseError", [A]);
            return undefined
        }
    }
    ...
    function i(y, z, w, x) {
        ...
        return {
            status: y.status, statusText: y.statusText, readyState: 1, open: function () {
            }, send: function () {
                x.fired = true;
                h.call(this, y, z, w)
            }, abort: function () {
    ...
    function k(C, A, w) {
        ...
        if (C.data && n.test(C.data) || n.test(C.url)) {
            q(C, A, w);
            var y = /^(\w+:)?\/\/([^\/?#]+)/, B = y.exec(C.url), z = B && (B[1] && B[1] !== location.protocol || B[2] !== location.host);
            C.dataType = "script";
    ...
    function t(z, x) {
        ...
        w.shift();
        var A = 0, y = w.length, C = z.urlParams.length, D = Math.min(y, C), B = {};
        for (A; A < D; A++) {
            var E = z.urlParams[A];
            B[E] = w[A]
        }
